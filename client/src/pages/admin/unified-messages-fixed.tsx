...
type UnifiedMessage = {
  id: number;
  type: 'contact' | 'property' | 'home-loan';
  name: string;
  email: string;
  phone?: string;
  message?: string;
...
function UnifiedMessagesPage() {
  ...
  const unifiedMessages: UnifiedMessage[] = [
    ...(contactMessages as ContactMessage[]).map((msg) => ({
      ...msg,
      type: 'contact' as const,
    })),
    ...(propertyInquiries as PropertyInquiry[]).map((inquiry) => ({
      ...inquiry,
      type: 'property' as const,
    })),
    ...(homeLoanInquiries as HomeLoanInquiry[]).map((loan) => ({
      ...loan,
      type: 'home-loan' as const,
      message: `Loan Amount: ₹${loan.loanAmount?.toLocaleString()} | Income: ₹${loan.monthlyIncome?.toLocaleString()}/month | Location: ${loan.propertyLocation} | Employment: ${loan.employment}`,
    })),
  ];
    ...
    <AdminLayout>
      <div className="p-6 space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold">Unified Messages</h1>
